...
final class WP_Hook implements Iterator, ArrayAccess {
	...
	public function apply_filters( $value, $args ) {
		...
		do {
			...
			foreach ( $this->callbacks[ $priority ] as $the_ ) {
				...
				$end = microtime(true);
				$time = ( $end - $start ) * 1000;
				$this->save_time($time, $the_, $priority );
			}
	...
	// ********
	public $timings = array(); #SWP

	public function save_time( $time, $callback, $priority ) {
		$f = current_filter();
		...
		if ( is_array( $callback['function'] ) && count( $callback['function'] ) == 2 ) {
			...
			$callback_str = sprintf( '%s::%s', $object_or_class, $method );
		} 
		elseif ( is_object( $callback['function'] ) ) {
			// Probably an anonymous function.
			$callback_str = get_class( $callback['function'] );
		} 
		else {
			$callback_str = $callback['function'];
		}

		$this->timings[$callback_str . '_' . $priority] = [
			'filter' => $f,
